  ...
  if (!currentUser ||!isAdmin) {
    return (
      <div className="max-w-xl mx-auto my-16 bg-white p-8 rounded-3xl border-2 border-rose-300 shadow-2xl text-center space-y-5">
        ...
        <div className="pt-2 flex flex-col gap-2"><button onClick={() => onNavigate('/admin/login')} className="w-full py-3 bg-[#0E6B4D] text-white font-bold rounded-xl">Admin Login এ যান</button><button onClick={() => onNavigate('/')} className="text-xs text-slate-500">হোম পেজে ফিরে যান</button></div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-lg font-bold text-slate-800 font-['Hind_Siliguri']">শ্রেণী ভিত্তিক নোটের বণ্টন (Notes per Class)</h3>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-lg font-bold text-slate-800 font-['Hind_Siliguri'] flex items-center justify-between"><span>সর্বশেষ আপলোডকৃত নোটসমূহ</span><button onClick={() => setActiveTab('manage')} className="text-xs text-[#0E6B4D] font-bold">সবগুলো ম্যানেজ করুন →</button></h3>
            {books.length === 0? <p className="text-xs text-slate-500 text-center py-6">কোনো নোট নেই। Add New Note ট্যাব থেকে নতুন নোট আপলোড করুন।</p> : <div className="space-y-2.5">{books.slice(0, 4).map((b) => <div key={b.id} className="p-3 bg-slate-50 rounded-xl flex items-center justify-between text-xs"><div><p className="font-bold text-slate-800 line-clamp-1">{b.title}</p><span className="text-slate-500">{b.class_label} • {b.subject}</span></div><span className={`px-2 py-0.5 rounded font-bold ${b.is_free? 'bg-emerald-100 text-[#0E6B4D]' : 'bg-amber-100 text-amber-800'}`}>{b.is_free? 'FREE' : `৳${b.price}`}</span></div>)}</div>}
          </div>
        </div>
      )}
      ...
      {activeTab === 'manage' && (
        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm space-y-4 p-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4"><h3 className="text-lg font-bold text-slate-800 font-['Hind_Siliguri']">নোট ম্যানেজমেন্ট তালিকা (Manage All Notes)</h3><button onClick={() => { resetForm(); setActiveTab('add'); }} className="px-4 py-2 bg-[#0E6B4D] text-white text-xs font-bold rounded-xl flex items-center gap-1.5"><PlusCircle className="w-4 h-4 text-[#FFB400]" /><span>নতুন নোট আপলোড</span></button></div>
          {books.length === 0? <div className="text-center py-12 space-y-3"><p className="text-sm text-slate-600 font-bold">কোনো নোট নেই।</p></div> : <div className="overflow-x-auto"><table className="w-full text-left text-xs border-collapse"><thead><tr className="bg-[#0E6B4D] text-white"><th className="p-3.5">নোটের শিরোনাম</th><th className="p-3.5">শ্রেণী</th><th className="p-3.5">বিষয়</th><th className="p-3.5">টাইপ / মূল্য</th><th className="p-3.5">ভিউ</th><th className="p-3.5 text-right">অ্যাকশন</th></tr></thead><tbody className="divide-y divide-slate-100 text-slate-700">{books.map((book) => <tr key={book.id} className="hover:bg-slate-50"><td className="p-3.5 font-bold max-w-xs truncate">{book.title}</td><td className="p-3.5">{book.class_label}</td><td className="p-3.5">{book.subject}</td><td className="p-3.5"><span className={`px-2 py-0.5 rounded font-bold ${book.is_free? 'bg-emerald-100 text-[#0E6B4D]' : 'bg-amber-100 text-amber-800'}`}>{book.is_free? 'FREE' : `৳${book.price}`}</span></td><td className="p-3.5">{book.views || 0}</td><td className="p-3.5 text-right space-x-2"><button onClick={() => handleEditClick(book)} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded font-bold">এডিট</button><button onClick={() => { if (confirm(`"${book.title}" মুছে ফেলতে নিশ্চিত?`)) { onDeleteBook(book.id); showToast('মুছে ফেলা হয়েছে!'); } }} className="px-2.5 py-1 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded font-bold">ডিলেট</button></td></tr>)}</tbody></table></div>}
        </div>
      )}

      {activeTab === 'add' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xl space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4"><div><h3 className="text-xl font-bold text-slate-900 font-['Hind_Siliguri']">{editingBookId? 'নোট সম্পাদনা করুন (Edit Note)' : 'নতুন নোট যোগ করুন (Add New Note)'}</h3><p className="text-xs text-slate-500">বাংলা শিরোনাম, সিলেবাস ও HTML কনটেন্ট ফিল্ড পূরণ করুন।</p></div><div className="flex items-center gap-2"><span className="text-xs font-bold text-slate-500">দ্রুত টেস্ট প্রিসেট:</span><button type="button" onClick={() => handleLoadPreset(0)} className="px-3 py-1.5 bg-emerald-100 hover:bg-emerald-200 text-[#0E6B4D] font-bold rounded-lg text-xs">+ SSC পদার্থবিজ্ঞান</button><button type="button" onClick={() => handleLoadPreset(1)} className="px-3 py-1.5 bg-amber-100 hover:bg-amber-200 text-amber-800 font-bold rounded-lg text-xs">+ HSC রসায়ন</button></div></div>
          <form onSubmit={handleFormSubmit} className="space-y-5 text-xs">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block font-bold text-slate-700 mb-1">নোটের শিরোনাম (Title Bengali) *</label><input type="text" required value={formData.title} onChange={(e) => handleTitleChange(e.target.value)} className="w-full px-3 py-2.5 border border-slate-300 rounded-xl text-sm font-semibold" /></div><div><label className="block font-bold text-slate-700 mb-1">Slug (URL Path) *</label><input type="text" required value={formData.slug} onChange={(e) => setFormData(prev => ({...prev, slug: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-300 rounded-xl font-mono text-xs" /></div></div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><div><label className="block font-bold text-slate-700 mb-1">শ্রেণী (Class Level) *</label><select value={formData.class_level} onChange={(e) => handleClassLevelChange(parseInt(e.target.value, 10))} className="w-full px-3 py-2.5 border border-slate-300 rounded-xl font-bold"><option value={6}>Class 6</option><option value={7}>Class 7</option><option value={8}>Class 8</option><option value={9}>Class 9-10 SSC</option><option value={11}>Class 11-12 HSC</option><option value={12}>Admission</option></select></div><div><label className="block font-bold text-slate-700 mb-1">বিষয় (Subject) *</label><input type="text" required value={formData.subject} onChange={(e) => setFormData(prev => ({...prev, subject: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-300 rounded-xl font-bold" /></div><div><label className="block font-bold text-slate-700 mb-1">অধ্যায় (Chapter)</label><input type="text" value={formData.chapter} onChange={(e) => setFormData(prev => ({...prev, chapter: e.target.value }))} className="w-full px-3 py-2.5 border border-slate-300 rounded-xl" /></div></div>
            <div><label className="block font-bold text-slate-700 mb-1">সংক্ষিপ্ত বিবরণ (Description)</label><textarea rows={2} value={formData.description} onChange={(e) => setFormData(prev => ({...prev, description: e.target.value }))} className="w-full px-3 py-2 border border-slate-300 rounded-xl" /></div>
            <div><label className="block font-bold text-slate-700 mb-1">মূল নোট কনটেন্ট (Content HTML) *</label><textarea rows={8} required value={formData.content_html} onChange={(e) => setFormData(prev => ({...prev, content_html: e.target.value }))} className="w-full p-3 font-mono text-xs border border-slate-300 rounded-xl" /></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
              <div className="space-y-1.5"><label className="block font-bold text-slate-800 flex gap-1.5"><FileText className="w-4 h-4 text-[#0E6B4D]" />PDF ফাইল *</label><div onDragOver={(e) => { e.preventDefault(); setPdfDragActive(true); }} onDragLeave={() => setPdfDragActive(false)} onDrop={(e) => { e.preventDefault(); setPdfDragActive(false); if (e.dataTransfer.files[0]) handlePdfUpload(e.dataTransfer.files[0]); }} className={`relative border-2 border-dashed rounded-2xl p-4 text-center ${pdfDragActive? 'border-[#0E6B4D] bg-emerald-50' : formData.pdf_url? 'border-emerald-500 bg-emerald-50/60' : 'border-slate-300 bg-slate-50'}`}><input type="file" accept=".pdf" onChange={(e) => e.target.files && handlePdfUpload(e.target.files[0])} className="absolute inset-0 opacity-0 cursor-pointer z-10" />{pdfUploading? <span>Uploading {pdfProgress}%</span> : formData.pdf_url? <div className="flex justify-between"><span className="truncate font-bold">{pdfFileName} ✓</span><button type="button" onClick={clearPdfUpload}><Trash2 className="w-3.5 h-3.5" /></button></div> : <span>PDF Drag & Drop</span>}</div>{pdfUploadError && <p className="text-rose-600">{pdfUploadError}</p>}</div>
              <div className="space-y-1.5"><label className="block font-bold flex gap-1.5"><Sparkles className="w-4 h-4 text-[#FFB400]" />Thumbnail</label><div onDragOver={(e) => { e.preventDefault(); setThumbDragActive(true); }} onDragLeave={() => setThumbDragActive(false)} onDrop={(e) => { e.preventDefault(); setThumbDragActive(false); if (e.dataTransfer.files[0]) handleThumbnailUpload(e.dataTransfer.files[0]); }} className={`relative border-2 border-dashed rounded-2xl p-4 text-center ${thumbDragActive? 'border-[#0E6B4D] bg-emerald-50' : formData.thumbnail_url? 'border-emerald-500 bg-emerald-50/60' : 'border-slate-300 bg-slate-50'}`}><input type="file" accept="image/*" onChange={(e) => e.target.files && handleThumbnailUpload(e.target.files[0])} className="absolute inset-0 opacity-0 cursor-pointer z-10" />{thumbUploading? <span>Uploading {thumbProgress}%</span> : formData.thumbnail_url? <img src={thumbPreview || formData.thumbnail_url} className="w-12 h-12 mx-auto rounded object-cover" /> : <span>Cover Image</span>}</div></div>
            </div>
            <div className="space-y-3 border-2 border-dashed border-[#FFB400] bg-amber-50/40 p-4 rounded-2xl">
              <label className="font-bold flex gap-2 text-sm"><ImageIcon className="w-4 h-4 text-[#0E6B4D]" /> Preview এর জন্য ৩টা ছবি আপলোড করুন (Detail Page এ সুন্দর Preview দেখাবে) - Max 6 টা</label>
              <div onDragOver={(e) => { e.preventDefault(); setPreviewDragActive(true); }} onDragLeave={() => setPreviewDragActive(false)} onDrop={(e) => { e.preventDefault(); setPreviewDragActive(false); handlePreviewImagesUpload(e.dataTransfer.files); }} className={`relative border-2 border-dashed rounded-2xl p-6 text-center ${previewDragActive? 'border-[#0E6B4D] bg-emerald-50' : 'border-slate-300 bg-white'}`}>
                ...
                {previewUploading? <span className="font-bold">Uploading {previewProgress}%...</span> : <div><Upload className="w-6 h-6 mx-auto mb-1" /><p className="font-bold">Preview ছবি Drag করুন বা Click করে Select করুন</p></div>}
              </div>
              {previewImages.length > 0 && <div className="grid grid-cols-3 gap-3 mt-3">{previewImages.map((url, idx) => <div key={idx} className="relative"><img src={url} className="w-full h-28 object-cover rounded-xl border" /><button type="button" onClick={() => removePreviewImage(idx)} className="absolute -top-2 -right-2 bg-rose-600 text-white rounded-full p-1"><X className="w-3 h-3" /></button></div>)}</div>}
            </div>
      ...
      {activeTab === 'users' && (
        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm p-6 space-y-4">
          <h3 className="text-lg font-bold text-slate-800 font-['Hind_Siliguri']">নিবন্ধিত ইউজার তালিকা (Registered User Profiles)</h3>
          <div className="overflow-x-auto"><table className="w-full text-left text-xs border-collapse"><thead><tr className="bg-[#0E6B4D] text-white"><th className="p-3.5">ইমেইল</th><th className="p-3.5">নাম</th><th className="p-3.5">রোল (Role)</th><th className="p-3.5">স্ট্যাটাস</th></tr></thead><tbody className="divide-y divide-slate-100 text-slate-700">{profiles.map((p) => <tr key={p.id || p.email} className="hover:bg-slate-50"><td className="p-3.5 font-mono font-bold">{p.email}</td><td className="p-3.5 font-bold">{p.full_name || 'Student'}</td><td className="p-3.5"><span className={`px-2 py-0.5 rounded font-bold ${p.email === ADMIN_EMAIL? 'bg-[#FFB400] text-[#0E6B4D]' : 'bg-slate-100 text-slate-800'}`}>{p.email === ADMIN_EMAIL? '👑 Owner Admin' : 'Student'}</span></td><td className="p-3.5 text-emerald-700 font-bold">Active</td></tr>)}</tbody></table></div>
        </div>
      )}

      {activeTab === 'purchases' && (
        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm p-6 space-y-4">
          <h3 className="text-lg font-bold text-slate-800 font-['Hind_Siliguri']">পেমেন্ট ও ক্রয়কৃত ট্রানজেকশন তালিকা (Purchases Log)</h3>
          {purchases.length === 0? <p className="text-xs text-slate-500 text-center py-6">এখনো কোনো ক্রয়কৃত পেমেন্ট হিস্ট্রি নেই।</p> : <div className="overflow-x-auto"><table className="w-full text-left text-xs border-collapse"><thead><tr className="bg-[#0E6B4D] text-white"><th className="p-3.5">তারিখ</th><th className="p-3.5">ইউজার ইমেইল</th><th className="p-3.5">নোট আইডি</th><th className="p-3.5">পরিমাণ</th><th className="p-3.5">TrxID</th></tr></thead><tbody className="divide-y divide-slate-100 text-slate-700">{purchases.map((p) => <tr key={p.id} className="hover:bg-slate-50"><td className="p-3.5">{new Date(p.created_at).toLocaleDateString()}</td><td className="p-3.5 font-mono">{p.user_email || p.user_id}</td><td className="p-3.5 font-bold">{p.book_title || p.book_id}</td><td className="p-3.5 font-bold text-[#0E6B4D]">৳{p.amount}</td><td className="p-3.5 font-mono">{p.trx_id}</td></tr>)}</tbody></table></div>}
        </div>
      )}

      {activeTab === 'settings' && (
        <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 space-y-6">
          <h3 className="text-xl font-bold text-slate-900 font-['Hind_Siliguri']">অ্যাডমিন সেটিংস ও ডাটাবেস কন্ট্রোল (Settings & Schema)</h3>
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2 text-xs"><span className="font-bold text-slate-800 block">অনুমোদিত Admin Email Environment Variable:</span><code className="bg-white px-3 py-1.5 rounded border border-slate-300 block font-mono font-bold text-[#0E6B4D]">VITE_ADMIN_EMAIL="{ADMIN_EMAIL}"</code><p className="text- text-slate-500">Note: আপনার আসল ইমেইল সেট করতে.env ফাইলে VITE_ADMIN_EMAIL মানটি আপডেট করুন।</p></div>
          <div className="pt-4 border-t border-slate-200 flex flex-wrap gap-4"><button onClick={onOpenSetupModal} className="px-5 py-2.5 bg-[#0E6B4D] hover:bg-[#0A523B] text-white font-bold text-xs rounded-xl flex items-center gap-2"><Database className="w-4 h-4 text-[#FFB400]" /><span>Supabase Schema & RLS SQL দেখুন</span></button><button onClick={() => { if (confirm('আপনি কি ডাটা ক্যাশ রিসেট করে শুরু করতে চান?')) { onResetSampleData(); showToast('ডাটাবেস ক্যাশ রিসেট করা হয়েছে!'); } }} className="px-5 py-2.5 bg-rose-100 hover:bg-rose-200 text-rose-800 font-bold text-xs rounded-xl flex items-center gap-2"><RefreshCw className="w-4 h-4" /><span>ডাটা রিসেট করুন (Reset Cache)</span></button></div>
        </div>
      )}
    </div>
  );
